subroutine evolution_particle_3D(ipu,time,time_prv,substep_max)
#include "macro.h"
  use simdata3D
  use particle_data
  use unit
  ...
  !$omp shared(ipu,flag_evol,time,time_prv,dt,var_p,x,y,z,vlx_b,vly_b,vlz_b,vlx,vly,vlz,ju,jd,ku,kd,lu,ld,lv_min,lv_max) &
  ...
  do ip=1,ipu
     if(flag_evol(ip)==1)then
        ! evolve the particle
        t    =  time_prv
        xtmp =  var_p(index_x,ip)
        ytmp =  var_p(index_y,ip)
        ztmp =  var_p(index_z,ip)
...
!            xtmp = max(min(xtmp,x(ju,lv_min)),x(jd,lv_min))
!            ytmp = max(min(ytmp,y(ku,lv_min)),y(kd,lv_min))
! #ifdef FULL
!            ztmp = max(min(ztmp,z(lu,lv_min)),z(ld,lv_min))
! #else
!            ztmp = max(min(ztmp,z(lu,lv_min)),0.d0)
! #endif
...
!!! interpolate variables
        ...
        var_p(index_x,ip) = xtmp
        var_p(index_y,ip) = ytmp
        var_p(index_z,ip) = ztmp
...
end subroutine evolution_particle_3d
