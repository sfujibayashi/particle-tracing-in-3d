subroutine sort_median(ntemp,np,np_active,fac_lw,fac_med,fac_rw,val_temp_traj_list,mass_temp_traj_list,val_lw_temp_list,val_med_temp_list,val_rw_temp_list)
  implicit none

  integer,intent(in) :: np, ntemp, np_active(ntemp)
  real(8),intent(in) :: fac_lw,fac_med,fac_rw
  real(8),intent(in) :: val_temp_traj_list(ntemp,np),mass_temp_traj_list(ntemp,np)
  real(8),intent(out) :: val_lw_temp_list(ntemp),val_med_temp_list(ntemp),val_rw_temp_list(ntemp)
  
  integer,allocatable :: ip_list(:)
  real(8) :: dummy_list(np)
  
  integer :: itemp, ip, ipp,ip_dummy, np_active_itemp
...
!     write(6,*) itemp
     ...
     ! do ip=1,np_active_itemp
     !    write(6,*) ip,dummy_list(ip), val_temp_traj_list(itemp,ip_list(ip))
     ! enddo
     ! stop
     ...
     enddo
     write(6,*) itemp, ip_lw, ip_med, ip_rw
