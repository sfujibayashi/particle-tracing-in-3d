...
contains
  ...
  subroutine make_histogram_rho_ye(t,it)
    ...
    real(8),allocatable :: histogram_rho_ye_total(:,:),histogram_rho_ye_outside_ah(:,:),histogram_rho_ye_ejecta(:,:)
    ...
    do lv=lv_min,lv_max
       ...
       !$omp   reduction(+:mass_tot,histogram_rho_ye_total,histogram_rho_ye_ejecta,histogram_ye_outside_ah,histogram_ye_ejecta,histogram_vel_ejecta,histogram_ye_ejectah,histogram_vel_ejectah)
       !$omp do
       do l=ld,lu
          do k=kd,ku
             do j=jd,ju
                ...
                if(dm>1d30)write(6,*) lv, j,k,l, qb(j,k,l,lv), vol3D(j,k,l,lv)
       ...
       !write(6,*) sum(histogram_vel_ejecta(:)), sum(histogram_ye_ejecta(:))
       !if(sum(histogram_vel_ejecta(:))>1d33)write(6,*) lv
    enddo
    
    !write(6,*) mass_tot,sum(histogram_vel_ejecta(:))
    !stop
    ...
    open(newunit=unit_num,file=trim(dir_out)//"/ye_histogram_"//trim(str1)//".dat",status="replace")
    ...
    open(newunit=unit_num,file=trim(dir_out)//"/vel_histogram_"//trim(str1)//".dat",status="replace")
